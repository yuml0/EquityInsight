...
function PortfolioPage() {
  ...
  // Validation logic for analytics access
  const canAccessAnalytics = useMemo(() => {
    // Check if there are companies in the portfolio
    if (companies.length === 0) {
      return false;
    }

    // Check if any company has a non-zero weight
    const hasNonZeroWeights = companies.some((company) => company.weight > 0);
    return hasNonZeroWeights;
  }, [companies]);

  // Handle tab switching with validation
  const handleTabChange = useCallback((value: string) => {
    if (value === "analytics" && !canAccessAnalytics) {
      return; // Prevent switching to analytics if not allowed
    }
    setActiveTab(value);
  }, [canAccessAnalytics]);

  // Auto-switch to portfolio if analytics becomes disabled while active
  useMemo(() => {
    if (activeTab === "analytics" && !canAccessAnalytics) {
      setActiveTab("portfolio");
    }
  }, [activeTab, canAccessAnalytics]);
            ...
            <TabsList className="grid w-full grid-cols-3 mb-6 flex-shrink-0">
              ...
              <TabsTrigger
                ...
                title={!canAccessAnalytics
                  ? companies.length === 0
                    ? "Add companies to your portfolio to access analytics"
                    : "Set non-zero weights for companies to access analytics"
                  : "View climate analytics for your portfolio"}
              >
                  ...
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center text-muted-foreground">
                      ...
                      <p className="text-sm mb-4">
                        {companies.length === 0
                          ? "Add companies to your portfolio to access climate analytics"
                          : "Set non-zero weights for your companies to access climate analytics"}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Go to the Portfolio Management tab to add companies and
                        set weights
                      </p>
                    </div>
                  </div>
